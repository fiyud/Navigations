...
class UnifiedAdvancedNoMaDTrainer:
    ...
    def pretrain_world_model(self, num_episodes: int = 50):
        ...
        for episode in range(num_episodes):
            ...
            while not done and episode_steps < self.config['max_episode_steps']:
                ...
                action_onehot = torch.zeros(self.config.get('action_dim', 6), device=self.device)
                action_onehot[action_idx] = 1.0
                action_onehot = action_onehot.unsqueeze(0)
                
                with torch.no_grad():
                    try:
                        outputs = self.model.forward(torch_obs, mode="all", prev_action=action_onehot)
                        value = outputs.get('values', torch.zeros(1, device=self.device))
                    except Exception as e:
                        print(f"Warning during pretraining: {e}")
    ...
    def _compute_world_model_loss(self, observations, actions, rewards):
        ...
        # Get features
        if actions.dim() == 1 or actions.size(-1) == 1:
            action_indices = actions.long().squeeze(-1)
            actions_onehot = torch.zeros(actions.size(0), self.config.get('action_dim', 6), device=self.device)
            actions_onehot.scatter_(1, action_indices.unsqueeze(1), 1.0)
        else:
            actions_onehot = actions
        
        outputs = self.model.forward(observations, mode="features", prev_action=actions_onehot)
    ...
    def train(self, total_timesteps: int):
        ...
        self.meta_learning_mode = self.config.get('use_meta_learning', True)

        if self.meta_learning_mode:
            self.meta_batch_size = self.config.get('meta_batch_size', 4)
            self.inner_loop_steps = self.config.get('inner_loop_steps', 1)
...
class ParallelCollector:
    ...
    def _worker_loop(self, worker_id: int, shared_model_state: Dict):
        """Worker process that collects experience"""
        env = self.env_fn()
        model = create_unified_model(self.config).to('cpu')
        
        model.load_state_dict(shared_model_state['model_state_dict'])
        model.eval()
...
def main():
    ...
    parser.add_argument('--no-pretrain', action='store_true',
                       help='Skip world model pretraining')
    ...
    if args.no_pretrain:
        config['pretrain_world_model'] = False
