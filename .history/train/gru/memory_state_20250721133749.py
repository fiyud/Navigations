...
class RSSMCore(nn.Module):
    """Recurrent State-Space Model from DayDreamer"""
    def __init__(self, feature_dim=256, action_dim=4, stoch_size=32, deter_size=512, hidden_size=512, num_classes=32):
        ...
        self.cell = nn.GRUCell(stoch_size * num_classes + action_dim, deter_size)
...
class SpatialMemoryGraphODE(nn.Module):
    """Neural ODE dynamics with RSSM for spatial memory graph evolution"""
    ...
    def forward(self, graph_state: 'SpatialMemoryGraphState', 
        ...
        """
        Update graph with new observation using RSSM dynamics
        """
        # Encode observation
        ...
        # Combine RSSM state with encoded features
        combined_features = torch.cat([
            post.stoch.reshape(post.stoch.shape[0], -1),
            post.deter
        ], -1)
        
        # Use combined features for graph update
        node_features = self.decoder(combined_features)
        
        # Update graph state (keep existing logic)
...
class UnifiedSpatialMemoryGraphODE(nn.Module):
    ...
    def forward(self, observation: torch.Tensor, position: torch.Tensor,
        ...
        if self.rssm_state is None:
            self.rssm_state = RSSMState(batch_size, device=device)
        
        if action is None:
            action = torch.zeros(batch_size, 6, device=device)
