...
class UnifiedAdvancedNoMaDRL(nn.Module):
    ...
    def forward(self, observations: Dict[str, torch.Tensor], mode: str = "policy",
            ...
            """
            batch_size = observations['rgb'].size(0) if 'rgb' in observations else observations['context'].size(0)
            device = observations['context'].device if 'context' in observations else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            ...
            elif 'goal_position' in observations:
                position = observations['goal_position']
            else:
            ...
            if torch.isnan(vision_features).any():
                ...
                vision_features = torch.nan_to_num(vision_features, 0.0)
            ...
            if hasattr(self, 'spatial_memory_graph') and self.spatial_memory_graph is not None:
                graph_state, graph_features = self.spatial_memory_graph(
                    vision_features, position, time_delta
                )
                
                # Combine visual and graph features
                combined_features = torch.cat([vision_features, graph_features], dim=-1)
            ...
            if prev_action is None:
                ...
                if prev_action.size(-1) == 1 or prev_action.dtype == torch.long:
                    action_indices = prev_action.long().squeeze(-1)
                    prev_action = torch.zeros(batch_size, self.action_dim, device=device)
                    prev_action.scatter_(1, action_indices.unsqueeze(1), 1.0)
            
            # Ensure prev_action is on correct device
            ...
            for b in range(batch_size):
                if goal_mask[b].item() < 0.5:  # Goal-conditioned
                    ...
                    goal_features_list.append(torch.zeros(1, 1000, device=device))
            ...
            # Handle batch size 1 specially for spatial_memory_graph
            if batch_size == 1:
                spatial_features, spatial_info = self.spatial_memory_graph(
                    observation=vision_features.squeeze(0) if vision_features.dim() > 2 else vision_features,
                    position=position.squeeze(0) if position.dim() > 1 else position,
                    action=prev_action.squeeze(0) if prev_action.dim() > 1 else prev_action,
                    goal_features=goal_features,
...
class UnifiedAdvancedTrainingWrapper:
 ...
 derministic=deterministic,
    ...
    def compute_losses(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.training and 'recent_trajectories' in batch:
            nav_loss = total_loss
            
            # Get gradients from navigation loss
            nav_grads = torch.autograd.grad(
                nav_loss, 
                [p for n, p in self.model.named_parameters() if 'learned_interaction' not in n],
                retain_graph=True,
                create_graph=True
            )
            
            # Get gradients from interaction loss
            interaction_loss = self.model.learned_interaction_loss(
                batch['recent_hidden_states'],
                batch['recent_action_probs']
            )
            
            int_grads = torch.autograd.grad(
                interaction_loss,
                [p for n, p in self.model.named_parameters() if 'learned_interaction' not in n],
                retain_graph=True,
                create_graph=True
            ...
            grad_similarity = 0
            for g1, g2 in zip(nav_grads, int_grads):
            ...
            grad_alignment_loss = -grad_similarity / len(nav_grads)
            losses['grad_alignment_loss'] = grad_alignment_loss
            total_loss += self.config.get('grad_alignment_coef', 0.1) * grad_alignment_loss
