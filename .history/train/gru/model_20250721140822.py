...
class UnifiedAdvancedNoMaDRL(nn.Module):
    ...
    def forward(self, observations: Dict[str, torch.Tensor], mode: str = "policy",
        ...
        position = observations.get('goal_position', torch.zeros(obs_img.size(0), 3, device=obs_img.device))
        ...
        if prev_action is None:
            prev_action = torch.zeros(batch_size, self.action_dim, device=obs_img.device)
        else:
            if prev_action.dim() == 1:
                prev_action = prev_action.unsqueeze(0)
            if prev_action.size(0) != batch_size:
                prev_action = prev_action.expand(batch_size, -1)
            prev_action = prev_action.to(obs_img.device)
